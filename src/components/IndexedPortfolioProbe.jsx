...
export function IndexedPortfolioProbe({ onFetchIndexedPortfolio }) {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "8px" }}>
            ...
            <div style={{ padding: "10px 12px", borderRadius: "10px", background: "rgba(2, 6, 14, 0.55)", border: "1px solid rgba(148, 163, 184, 0.08)" }}>
              ...
              <div style={{ color: "#3b82f6", fontWeight: 700, fontSize: "16px", ...mono }}>{valueText(parsed.positions.reduce((sum, item) => sum + Number(item.valueUsd || 0), 0))}</div>
            </div>
            <div style={{ padding: "10px 12px", borderRadius: "10px", background: "rgba(2, 6, 14, 0.55)", border: "1px solid rgba(148, 163, 184, 0.08)" }}>
              <div style={{ fontSize: "8px", color: "#475569", letterSpacing: "1px", ...mono }}>VALOR SPOT</div>
              <div style={{ color: "#22c55e", fontWeight: 700, fontSize: "16px", ...mono }}>{valueText(parsed.assets.reduce((sum, item) => sum + Number(item.valueUsd || 0), 0))}</div>
            </div>
          </div>
              ...
              {parsed.positions.length ? parsed.positions.map((item) => (
                <div key={item.id} style={{ padding: "12px", borderRadius: "12px", background: "rgba(2, 6, 14, 0.5)", border: "1px solid rgba(148, 163, 184, 0.08)" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "8px" }}>
                    ...
                    <div style={{ color: "#22c55e", ...mono }}>{item.fees0 || item.fees1 ? `${fmt(item.fees0, 6)} ${item.fee0Symbol} / ${fmt(item.fees1, 6)} ${item.fee1Symbol}` : "—"}<div style={{ fontSize: "9px", color: "#64748b" }}>fees token</div></div>
