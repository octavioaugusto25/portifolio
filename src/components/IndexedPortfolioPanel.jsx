...
function classifyAsset(item) {
  ...
  return {
    trusted,
    isSpam: suspiciousName || (tiny && !trusted),
  };
}
...
function MetricBox({ label, value, color = "#e2e8f0" }) {
  return (
...
export function IndexedPortfolioPanel({ onFetchIndexedPortfolio, onUseIndexedPositions, onUseIndexedAssets, defaultWallet = "0x7828319afdffb75f26a54e941c787b5f11a9ee34" }) {
    ...
    <Card glow="#0ea5e9">
      ...
      {!result ? (
        <div style={{ fontSize: "11px", color: "#64748b", lineHeight: 1.6 }}>
          Use este bloco para puxar um portfolio indexado por carteira, com posicoes LP e ativos spot em varias redes.
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: "12px" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              <div style={{ fontSize: "9px", color: "#64748b", letterSpacing: "1px", ...mono }}>POSICOES INDEXADAS</div>
              {parsed.positions.length ? parsed.positions.map((item) => (
                <div key={item.id} style={{ padding: "14px", borderRadius: "14px", background: "linear-gradient(180deg, rgba(7, 12, 22, 0.9), rgba(3, 8, 18, 0.88))", border: "1px solid rgba(59,130,246,0.12)" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "8px" }}>
                    ...
                    <MetricBox label="FEES USD" value={item.feeUsd > 0 ? usdText(item.feeUsd) : "$0,00"} />
                  </div>
                </div>
